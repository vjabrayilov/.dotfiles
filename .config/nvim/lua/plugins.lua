...
return require('packer').startup(function(use)
    ...
    use { 'gauteh/vim-cppman' }              -- cpp docs
    use { 'christoomey/vim-tmux-navigator' } -- tmux navigation
    use { "szw/vim-maximizer" }              -- maximize and restore the current split windw
    -- 2 essential plugins
    use { 'tpope/vim-surround' }
    use { 'vim-scripts/ReplaceWithRegister' }
    use { 'numToStr/Comment.nvim' }                                  -- efficient (un)commeting; binding --> gc
    use { 'nvim-lua/plenary.nvim' }                                  -- lua functions use this plugin
    use { 'nvim-tree/nvim-tree.lua' }                                -- file explorer
    use { 'kyazdani42/nvim-web-devicons' }                           -- vscode like icons
    use { 'nvim-lualine/lualine.nvim' }                              -- fancy statusline
    use { 'nvim-telescope/telescope-fzf-native.nvim', run = 'make' } -- fzf dependency
